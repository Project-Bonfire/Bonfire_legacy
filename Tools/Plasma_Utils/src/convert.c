...
MODIFIED 01.12.16 by Karl Janson (karl.janson@ati.ttu.ee)
...
const char *argp_program_version = "MIPS-I binary format converter for Plasma v0.1";
const char *argp_program_bug_address = "karl.janson@ati.ttu.ee";
static char doc[] = "MIPS-I binary format converter for Plasma";
static char args_doc[] = "";
static struct argp_option options[] = {
   { "input", 'i', "<ELF_file>", 0, "MIPS-I ELF machine code file to be converted."},
   { "bin", 'b', "<BIN_file>", 0, "Generate a raw binary file."},
   { "txt", 't', "<TXT_file>", 0, "Generate a Plasma RAM image in text format (for HDL simulation)"},
   { 0 }
};
...
static error_t parse_opt(int key, char *arg, struct argp_state *state) {
   ...
   switch (key) {
   case 'i': arguments->elf_file = arg; break;
   case 'b': arguments->bin_file = arg; break;
   case 't': arguments->txt_file = arg; break;
...
int main(int argc, char *argv[])
{
  ...
  arguments.bin_file = NULL;
  arguments.txt_file = NULL;

  argp_parse(&argp, argc, argv, 0, 0, &arguments);

  if (arguments.elf_file == NULL)
  {
    printf("Error: Input ELF file not specified!\n");
    return -1;
  }

  if (arguments.bin_file == NULL && arguments.txt_file == NULL)
  {
    printf("Error: You need to specify at least one of the output formats!\n");
    return -1;
  }

  infile = fopen(arguments.elf_file, "rb");
  if(infile == NULL)
  {
  ...
  /* Write BIN file */
  if (arguments.bin_file != NULL)
  {
    outfile = fopen(arguments.bin_file, "wb");
    fwrite(code, length, 1, outfile);
    fclose(outfile);
  }

  /* Write TXT file */
  if (arguments.txt_file != NULL)
  {
    txtfile = fopen(arguments.txt_file, "w");
